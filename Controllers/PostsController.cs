...
[Route("api/v1")]
public class PostsController(ApplicationDbContext db, IApiEntityService entities, ICurrentUser currentUser) : ApiControllerBase(db, entities, currentUser)
{
    [HttpGet("feed")] public async Task<ActionResult> Feed() => await List(Db.Posts.Where(x => !x.IsDraft).OrderByDescending(x => x.CreatedAt));
    ...
    [HttpGet("posts/{id:int}")] public async Task<ActionResult> GetPost(int id) => Ok(await Db.Posts.FindAsync(id));
    ...
    [HttpPost("posts/{id:int}/react")] public async Task<ActionResult<Reaction>> ReactPost(int id, ReactionRequest request) => await Create<Reaction>([], x => { x.PostId = id; x.UserId = Me; x.Type = request.Type; });
    ...
    [HttpGet("posts/{id:int}/comments")] public async Task<ActionResult> Comments(int id) => await List(Db.Comments.Where(x => x.PostId == id && x.ParentCommentId == null));
    ...
    [HttpPost("comments/{id:int}/react")] public async Task<ActionResult<Reaction>> ReactComment(int id, ReactionRequest request) => await Create<Reaction>([], x => { x.CommentId = id; x.UserId = Me; x.Type = request.Type; });
